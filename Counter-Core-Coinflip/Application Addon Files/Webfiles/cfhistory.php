                ...
                <thead>
                     <tr>
                       ...
                       <th>Winner</th>
						...
						$i=0;
            $rs = mysql_query("SELECT * FROM `cflobbies` WHERE `ended`='2' ORDER BY `id` DESC LIMIT 15 ");
            while($row = mysql_fetch_array($rs))
            {

              $id = $row['id'];
              $p1 = fetchinfo("hid","cflobbies","id",$id);
              $p2 = fetchinfo("cid","cflobbies","id",$id);
              $hside = fetchinfo("hside","cflobbies","id",$id);
              $winside = fetchinfo("win","cflobbies","id",$id);
              $wageamount = fetchinfo("value","cflobbies","id",$id);
              $side = fetchinfo("hside","cflobbies","id",$id);
              $status = fetchinfo("ended","cflobbies","id",$id);


              if($winside == 1){$sidetext ='<img src="images/ct-coin.png" width="25">';}else{$sidetext = '<img src="images/t-coin.png" width="25">';}

              $winner = "";
              if($hside == $winside){$winner =  $p1;}else{$winner = $p2;}

              $winname = fetchinfo("name","users","steamid",$winner);
              $winimage = fetchinfo("avatar","users","steamid",$winner);

              echo'
              <tbody>
              <td><img src="'.$winimage.'" class="img-circle" width="25"> '.$winname.'</td>
              <td><a href="lobby.php?id='.$id.'" target="_blank">Over</td>
              <td>'.$wageamount.'</td>
              <td>'.$sidetext.'</td>
              ';
             }
